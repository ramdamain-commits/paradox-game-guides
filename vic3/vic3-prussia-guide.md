...
1.13 は海軍刷新と日本（The Great Wave DLC）が主題。プロイセン固有のコンテンツ追加はないが、外交・指揮官系の仕様変更が統一戦争の進め方に影響する。[src: Patch_1.13 wiki]

| 変更 | プロイセンへの影響 |
|------|------|
| 海軍刷新（艦船デザイナー、旗艦、海戦システム改編、新型艦20種） | プロイセンは陸軍主体のため直接の影響は小。ただし英仏墺の海軍が強化されるため、海軍最低限方針は維持しつつ封鎖・砲撃外交への対処は要確認 [src: Patch_1.13 wiki] |
| 砲撃外交（Gunboat Diplomacy）の追加 | 沿岸国相手のみ使える新外交アクション。プロイセンも条件次第で利用可能 [src: Patch_1.13 wiki] |
| 戦略的関心度（Strategic Interest）のティアド化 | 列強の介入閾値が見えやすくなる。統一戦争の外交プレイで第三国の動向を予測しやすい [src: Patch_1.13 wiki] |
| クーデター扇動（外交アクション追加） | ドイツ統一の小国取り込み戦略で補助手段になり得る [src: Patch_1.13 wiki] |
| 指揮官の単一化（複数指揮官制 → 単一指揮官制） | 軍編成あたり指揮官1名に変更。少数精鋭の将軍運用が基本。詳細は「軍事ドクトリン > 将軍の選び方」参照 [src: Patch_1.13 wiki] |
| プロミネンス（政治人物の影響力）導入 | IG 指導者選出の主因が Popularity から Prominence に変更。詳細は「内政・経済 > 利益団体管理 > プロミネンスの活用」参照 [src: Patch_1.13 wiki] |
...
- 植民地獲得（アフリカ分割）
- **海軍の整備（1.13 仕様）**: 艦船デザイナー（Ship Designer）で装甲・武装・推進力・補給能力をカスタマイズ。プロイセン/ドイツは陸軍主体だが、植民地維持と通商保護のため最低限の主力艦に **旗艦（Flagship）** を1隻指定して威信獲得を狙う [src: Patch_1.13 wiki]
- 社会制度の整備（社会保障、教育、医療）
- 大戦への備えまたは回避
...
1.13 で各政治家に **Prominence** ステータスが追加された。これは大衆人気（Popularity）とは別で、IG 内の政治力寄与度と次期指導者の選出確率に直接影響する。[src: Patch_1.13 wiki]
...
- プロイセンの場合、ビスマルク（`PRU_otto_von_bismarck`）は `german_unification.3` イベントで「首相任命」の経路で登用される。1.13 で IG 指導者選出は Prominence 主導になったが、ビスマルクの所属 IG・Prominence 値はスクリプトでの裏取り未実施のため、本ガイドでは IG 指導者として育てる手順は推奨しない（首相任命ルートを優先する）
...
| 手段 | プロイセンでの使い道 |
|------|---------------------|
| **戦略的関心度（Strategic Interest）のティアド化** | 列強の介入閾値が階層化された。北ドイツ圏に対する英仏墺の関心度ティアを確認し、低い時期を狙って外交戦を起こす [src: Patch_1.13 wiki] |
| **クーデター扇動（Orchestrate Coup）** | 友好的なロビー（Lobby）を持つ弱小ドイツ小国に対して、政体ごと親プロイセン政権に挿げ替える補助手段。直接戦争・外交戦より悪名コストが低い場合がある [src: Patch_1.13 wiki]（コミュニティ知見：具体的な発動条件・コストはスクリプト未確認） |
| **砲撃外交（Gunboat Diplomacy）** | デンマーク等の沿岸国相手に条約交渉時の脅迫オプションとして利用可能。180日間の海上敵対行動が許可される。プロイセンは海軍が弱いため過信は禁物 [src: Patch_1.13 wiki]（コミュニティ知見：必要海軍規模・威信要件はスクリプト未確認） |
| **条約システム強化** | Ship Transfer / Straits / Piracy 系の条約が追加。北海・バルト海の通行料管理に間接的に影響する可能性 [src: Patch_1.13 wiki] |

> **発動条件の裏取りメモ**: クーデター扇動・砲撃外交の前提条件・コストはパッチノートからの推測ベース。実プレイでの検証または `common/diplomatic_actions/` のスクリプト確認後に詳細化する予定。

#### UI導線（一般知識ベース）

- **クーデター扇動**: 対象国の外交画面（Diplomacy）→ 行動メニューに "Orchestrate Coup" が追加されている。条件未達の場合はグレーアウトでツールチップに不足要素が表示される想定
- **砲撃外交**: 条約交渉ダイアログ（Treaty Negotiation）の中で、相手が拒否しそうな場合に "Threaten Naval Hostilities" のチェックボックス/オプションが表示される
- 詳細な UI 配置は実プレイで確認後に追記する
...
| 海軍 | 最低限。海戦は避ける方針。1.13 では艦船デザイナーで装甲・武装をカスタマイズ可能だが、プロイセンの優先は陸軍にある [src: Patch_1.13 wiki] |

### 将軍の選び方（1.13 単一指揮官制）

1.13 で軍編成（Military Formation）あたりの指揮官は **1名のみ**になった。階級（Commander Rank）の指揮限界が技術で拡張されるため、**少数精鋭の将軍を昇進させて運用するのが基本**。[src: Patch_1.13 wiki]

- 軍事スキル + 特性（trait）の質を最優先。多数の凡庸な将軍より、優秀な将軍1〜2名のほうが効果的
- 利益団体との繋がりも確認（軍部出身が安定）
- 不必要な昇進は軍部 IG の承認低下ペナルティを招くので、必要になってから階級を上げる [src: Patch_1.13 wiki]
- **指揮官は編成間で再配置可能だが移動に時間がかかる**。移動中は階級分の指揮限界しか提供せず、特性ボーナスは失われる点に注意 [src: Patch_1.13 wiki]
...
- **複数前線になる場合は編成を分け、各編成に専任の優秀な将軍を割り当てる**（将軍の途中移動はボーナス喪失で損失大） [src: Patch_1.13 wiki]
- 同時攻撃可能な州数は将軍の階級と距離に依存する。前線が広い場合は階級の高い将軍を主戦場に置く [src: Patch_1.13 wiki]

#### UI導線（一般知識ベース）

- **編成画面**: 軍事タブ（Military）→ Formations 一覧 → 編成を選択して指揮官スロットに将軍をドラッグまたはクリック割り当て
- **将軍の昇進**: 同じ Formations 画面の指揮官カード上で階級アップのボタンが表示される（Authority コスト消費）
- 詳細な UI 配置は実プレイで確認後に追記する
...
| 従属国 | Subject | 従属国の総称 |
| プロミネンス | Prominence | 1.13追加。政治家の政治機構内での影響力。IG指導者選出の主因 |
| 艦船デザイナー | Ship Designer | 1.13追加。装甲・武装・推進力・補給能力をカスタマイズ可能 |
| 旗艦 | Flagship | 1.13追加。艦隊の威信獲得/喪失に関わる特別な艦 |
| 戦略的関心度 | Strategic Interest | 1.13でティアド化。地域への関与度の階層指標 |
| クーデター扇動 | Orchestrate Coup | 1.13追加の外交アクション。ロビー経由で政権転覆を狙う |
| 砲撃外交 | Gunboat Diplomacy | 1.13追加。条約交渉時に海上敵対行動を脅迫オプションとして提示 |

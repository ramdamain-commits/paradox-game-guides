...
- **他ガイドとの「訳語不統一」指摘を見たら、多数決（他N本の表記に合わせる）で解決する前に、ゲーム内ローカライズの `game_concepts_l_japanese.yml` 等で正式訳を1件は裏取りする**。多数派の表記自体が誤訳のケースがあり、多数決では誤りが温存される。実例（2026-07-13 EU5 suggestion級レビュー対応）: ottomanの用語対照表「大国指標｜Dominance」を修正しようとした際、他ガイド（austria/brandenburg）で使われていた「大国スコア」に合わせるだけでは不十分と判断し実際にローカライズファイルを検証したところ、`game_concept_great_power_score_short = "列強スコア"` が正式訳であり、多数派表記の「大国スコア」自体が誤訳と判明。austria4箇所・brandenburg2箇所・ottoman1箇所に加え、日本語併記が欠けていたgovernment/hungary/regional/universalの4ファイルも含め計10ファイルの訂正が必要になった
...
- **このマーカー昇格漏れパターンは、マーカー昇格に限らずあらゆる数値・分類・帰属の訂正で再発する**。1箇所（多くは本文の解説セクション）を修正しても、同じクレームが要約表・推奨編成表・固有進歩一覧表・用語対照表に再掲されている場合、そちらは修正時のgrep対象から漏れやすい。訂正を1件行ったら**必ず対象語句・数値で`grep -n`をガイド全体にかけ、表・要約セクションへの伝播漏れがないか確認する**のを訂正作業の最終ステップとして固定する。実例（2026-07-12 EU5 6ガイド見直しレビュー）: austria固有進歩表の効果値「-50%」が本文2箇所は「-33%」に訂正済みなのに表だけ旧値残存、hungary兵科推奨編成表・地形適性表・Combined Arms節でフサールが「重装騎兵」表記のまま（軍事ドクトリン章・用語対照表は`Light Cavalry`に訂正済み）と自己矛盾。いずれも過去のフェーズで一度「修正済み」と扱われたガイドで、2巡目のレビューで発覚した
- **パッチ更新・見直し時、「〇〇パッチで新設された」という帰属クレームは、記載前にWikiのパッチ履歴ページ（`Patches`一覧やメカニクス個別ページ）で実際の導入パッチを確認する**。スクリプトの実値検証だけでは「その値が正しいか」は確認できても「いつ導入されたか」は検証できないため、値が合っていても帰属が誤っている（＝既存メカニクスを新パッチ起源と誤記する）ケースを見逃す。実例（2026-07-12 CK3 1.19最新化）: ck3-systems-guideの「1.19主要変更サマリー」表8項目中6項目が帰属誤り（正統性=実際はPatch1.12 Scythe/封臣スタンス=1.9/戴冠式と誓約=1.17/遊牧連合=1.16/徴税スロット=1.11由来で、いずれも1.19より1〜5年前に導入済みの既存メカニクス）。「2026-04-09確認・1.19.0.1」という確認日付・パッチ版の記載自体は正確でも、個々の機能の導入時期の裏取りをしていなかったため誤りが6件連鎖した

...
## 2026-07-13 EU5 austriaガイド分割（サイズ突出対応）

### Changed
- テクニカルライターレビュー（2026-07-12）で指摘されていたaustriaガイドのサイズ突出（126,449字、次点hungaryの87,250字より+45%）に対応
- 「神聖ローマ帝国運営（オーストリア特化）」「ハプスブルク婚姻外交」「宗教改革への対応」の3セクション（計約22,000字）を新規ファイル [eu5-austria-hre-guide.md](eu5/eu5-austria-hre-guide.md) へ分離。本編には各セクション要点3〜5行+分離先へのリンクのみ残した
- 分離後のaustria本編は107,221字（hungary比+23%）に縮小。3セクションとも本文に「オーストリア特化」と明記されている通り他ガイドから参照されていなかったため、cross-guideのリンク張り替えは不要だった
- 本編中に残っていた前方参照の地の文（「ハプスブルク婚姻外交セクションで解説する」等2箇所）を新規ファイルへのリンクに修正
- `index.html` に新規ファイルのカードを追加（タグ「拡張」で本編「攻略」と区別）

### Notes
- 分割してもhungary比+23%差は残るが、austria自体の基本セクションが他ガイドより詳細に書かれていることが主因で、これ以上の圧縮は内容量の削減（=情報価値の毀損）を伴うため見送り
- CLAUDE.mdの「全書き直しは最終手段」ルールに沿い、sed/awkでのファイル分割（head/tail結合）で対応し全文書き直しは回避した

## 2026-07-13 EU5 国別6ガイド suggestion級指摘対応・列強スコア誤訳訂正

### Changed
- 前回レビュー（2026-07-12）で残っていたsuggestion級12件を全件対応
- **austria**: 帝国法ロードマップの信頼性マーカー混在行（verified出典+未検証コメント同居）を2文に分割。用語対照表の「都市特権（Urban Rights）」が本文未使用だったため、内政・経済節に短い解説と`eu5-castile-guide.md`への参照を追加
- **byzantium**: 見出し表記を他5ガイドに統一（「よくある失敗パターン」→「よくあるミス」、「用語表」→「用語対照表」）。出典セクションの「参照ファイル」「コミュニティ知見」重複2小節を1小節に統合。出典末尾の孤立した断片行「上記出典テーブルを参照」を削除。用語対照表の「ロマノス国境地理」を「ローマ国境地理」に訂正（人名ロマノスと紛らわしい誤訳）。「反ジェノヴァ盟約」の英語列に日本語説明文が入っていたのをスクリプトID表記に修正
- **castile**: 用語対照表（単一フラットテーブル）を「カスティーリャ固有用語」「共通用語（抜粋）」の2小節に分割
- **brandenburg**: 召集軍比較表ヘッダー「正規（Regulars）」を本文表記「正規軍」に統一
- **列強スコア（Great Power Score）誤訳の横断訂正（当初のsuggestion対応中に発覚・想定より広範囲）**: ottomanの用語対照表「大国指標｜Dominance」が誤り（該当システムはゲーム内に存在しない）と判明し検証したところ、ゲーム内`game_concepts_l_japanese.yml:1893`の正式訳は「列強スコア」であり、austria・brandenburgで使っていた「大国スコア」も誤訳と確定。austria4箇所・brandenburg2箇所・ottoman1箇所を「列強スコア」に統一し、日本語併記が欠けていたgovernment-guide/hungary/regional-guide/universal-guideの初出箇所にも追記。中央`localization-reference.md`に`Great Power`/`Great Power Score`を新規登録
- 修正後、全EU5ガイドを「大国スコア」「大国指標」で横断grepし伝播漏れがないことを確認

### Notes
- suggestion級12件は前回メモの「10件」より多かった（数え方の差異、writer/playerレビューの重複を除いた実数）
- 列強スコア誤訳は当初のレビューでは指摘されていなかった新規発見。ottomanの1件を裏取りする過程で他5ファイルへの誤訳波及が判明した（個別修正の伝播漏れパターンとは逆に、初出時の誤訳が複数ガイドへ独立混入したケース）

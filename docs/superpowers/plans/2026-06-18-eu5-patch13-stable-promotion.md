...
### Task 2.5-1: ottoman 拡充 — ✅完了 (07-12・65.4K→81.0K字)
- [x] **「地中海制海戦略」を 10.5 独立セクションとして新設**（エーゲ海・黒海・紅海の三段展開）。1.3 船コスト増を踏まえた海軍規模設計をここに集約。
- [x] **固有イベント時系列の完全テーブル化**（カリフ制宣言・スレイマン大帝連鎖・衰退イベント連鎖を全件 ID・条件・推奨選択・効果で表化）。現状は外交章に飛ばされ時系列テーブルが薄い。
- [x] ティマール制をサブ節から独立節に格上げ＋デヴシルメ制節を新設。

### Task 2.5-2: castile 拡充 — ✅完了 (07-12・53.3K→75.4K字)
- [x] **固有イベント時系列テーブルの拡充**（イベリアの結婚・コムネロスの乱・異端審問・著名軍人を全件表化。選択肢と分岐条件を明記）。現状 54 行（austria の約 1/4）。
- [x] **「異端審問所運営」を 10.5 節として独立化**（Inquisitor 廷臣メカニクスと長期運営・Papal Authority 運用を組み合わせた詳細）。
- [x] 植民地運営の「交易所 vs 植民地化の判断基準」を建造物優先順位テーブルで可視化。

### Task 2.5-3: brandenburg（プロイセン）拡充 — ✅完了 (07-12・59.6K→79.8K字)
- [x] **「大選帝侯改革イベント」を固有イベント時系列テーブルに完全統合**（分岐条件・推奨選択を表化）。現状終盤章に埋没。
- [x] **内政・経済章に HRE 諸侯固有の収入構造節を新設**（帝国課税との兼ね合い・選帝侯特権の経済的活用）。現状 30 行弱で最小クラス。
- [x] 「プロイセン建国後の内政戦略」節を追加（Age4〜6 の固有進歩と建造物の相互作用ロードマップ）。

### Task 2.5-4: 検証 — ✅完了 (07-12)
- [x] 拡充後に再実測: ottoman 81.0K / castile 75.4K / brandenburg 79.8K 字。全て religion 水準（52K・※旧計画の37Kは統合前計測）を超過し byzantium(79K)/hungary(85K) の中大型帯へ到達。新規数値は `[src:]`/`（コミュニティ知見）` 付与、固有イベント ID は実機 grep 裏取り済み。
- [x] **章追加4点セット**（用語表・出典・パッチ差分・CHANGELOG）反映。index.html は3カード共フェーズ2で「Patch 1.3」表記・betaバッジ除去済みのため無変更（拡充はカードメタ非影響）。reviewer 3並列指摘（castile コムネロス軸の既存行未昇格=critical／severe語彙統一／brandenburg 固有進歩件数 24→26 訂正）をメイン Edit で反映。beta/仮登録 grep 0件維持。paradox 4690013 push・root bc22567 でサブモジュール参照更新。
